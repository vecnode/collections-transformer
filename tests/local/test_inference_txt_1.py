#!/usr/bin/env python3
"""Local smoke test: summarise a text file using Ollama (3 sentences)."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Summarise a text file in 3 sentences using Ollama."
    )
    parser.add_argument(
        "--text",
        default=str(REPO_ROOT / "assets" / "test_txt_1.txt"),
        help="Path to a local text file to summarise (default: assets/test_txt_1.txt).",
    )
...
def main() -> None:
    ...
    system_prompt = (
        ...
        "Summarise the provided text in exactly 3 sentences. "
        "Do not add commentary or headings — return only the summary."
    )
